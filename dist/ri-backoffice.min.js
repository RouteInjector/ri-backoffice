...
(function() {
    ...
    angular.module("injectorApp").factory("common", [ "$rootScope", function($rootScope) {
        ...
        factory.parsePathParams = function(path) {
            ...
            return {
                path: parts[0],
                vars: parts.length > 1 ? parts[1].split("&").reduce(function(acum, kv) {
                    var val = kv.split("=");
                    acum[val[0]] = val[1];
                    return acum;
                }, {}) : {}
...
(function() {
    ...
    angular.module("injectorApp").directive("sideMenu", function() {
        return {
            ...
            controller: [ "$scope", "$routeParams", "$location", "common", "models", "customMenu", "loginProvider", "$window", "$rootScope", function($scope, $routeParams, $location, common, models, customMenu, loginProvider, $window, $rootScope) {
                $scope.common = common;
...
(function() {
    "use strict";
    angular.module("injectorApp").directive("modelButtons", [ "$routeParams", "$http", "$q", "$route", "models", "$location", "$rootScope", "$httpParamSerializer", "common", function($routeParams, $http, $q, $route, models, $location, $rootScope, $httpParamSerializer, common) {
        return {
            restrict: "AE",
            scope: false,
            ...
            link: function(scope, element, attrs, ngModel) {
                ...
                function isActionAllowed(action, _currentShard) {
                    ...
                    if (action.allowedShards) {
                        if (Array.isArray(action.allowedShards)) return currentShard ? action.allowedShards.includes(currentShard) : false; else if (action.allowedShards === "*") return currentShard ? true : false; else return false;
                    } else {
                        return true;
                    }
                }
                scope.performAction = function(action) {
                    ...
                    if (isActionAllowed(action, currentShard)) {
                        var shardField;
                        var objParams = {};
                        if (action.shardMode) shardField = action.shardField || "__RI-currentShard";
        ...
        models.getModel($scope.schema, function(model) {
            ...
            $scope.getUrl = function(element, schema) {
                if ($scope.config.viewURL) {
                    var url = $scope.config.viewURL.replace("{{id}}", encodeURIComponent(encodeURIComponent($scope.id(element)))).replace("{{shardKey}}", encodeURIComponent($scope.shard(element)));
                    return url;
                }
                var model;
                if (element.__t) {
...
(function() {
    ...
    angular.module("injectorApp").controller("ShardingController", [ "$scope", "$routeParams", "$rootScope", "models", "configs", function($scope, $routeParams, $rootScope, models, configs) {
        ...
        $scope.setShard = function(value, notEmit) {
            ...
            if (!notEmit) $rootScope.$broadcast("shardChangeEvent");
        };
