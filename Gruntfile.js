module.exports = function (grunt) {
  ...
  grunt.config(
    'env',
    grunt.option('env') || process.env.GRUNT_ENV || 'production'
  );
  ...
  var buildProperties = {
    ...
    srcPath: 'src/',
    bowerFolder: '<%= config.srcPath %>bower_components/',
    injectorFolder: '<%= config.srcPath %>injector-assets/',
  ...
  grunt.initConfig({
    ...
    jshint: {
      ...
      uses_defaults: [
        '!<%= config.injectorFolder %>js/livereload.js',
        '<%= config.injectorFolder %>js/**/*.js',
        '<%= config.injectorFolder %>js/**/**/*.js',
      ],
    },
    sass: {
    ...
    watch: {
      ...
      js: {
        files: ['<%= config.srcPath %>**/*.js', '!<%= config.distFolder %>/**'],
    ...
    clean: {
      ...
      postmin: {
        src: [
          '<%= config.distFolder %>*-libs.js',
          '<%= config.distFolder %>*.min.temp.js',
        ],
      },
    },
    ngAnnotate: {
        ...
        files: [
          {
            src: [
              ...
              '<%= config.injectorFolder %>js/controllers/galleryController.js',
        ...
        files: [
          {
            src: [
              ...
              '<%= config.srcPath %>schema-form-assets/angular-schema-form-textarea/dist/bootstrap-textarea.js',
              '<%= config.srcPath %>schema-form-assets/angular-schema-form-password/dist/bootstrap-password.js',
              '<%= config.srcPath %>schema-form-assets/angular-schema-form-seconds/dist/bootstrap-seconds.js',
              '<%= config.srcPath %>schema-form-assets/angular-schema-form-multiselect/dist/bootstrap-multiselect.js',
              '<%= config.srcPath %>schema-form-assets/angular-schema-form-rating/dist/bootstrap-rating.js',
              // '<%= config.srcPath %>schema-form-assets/angular-schema-form-ritinymce/bootstrap-ritinymce.js',
              '<%= config.srcPath %>schema-form-assets/angular-schema-form-tinymce/dist/bootstrap-tinymce.js',
              '<%= config.bowerFolder %>angular-punch-card/dist/punch-card.js',
              '<%= config.bowerFolder %>json-human/src/json.human.js',
              '<%= config.srcPath %>angular-flash/angular-flash.js',
              '<%= config.srcPath %>ngf-upload-patched/ng-file-upload-all.js',
            ],
            dest:
    ...
    uglify: {
      options: {
        banner:
          '/*! <%= config.appName %> <%= pkg.name %> <%= grunt.template.today("yyyy-mm-dd") %> */\n',
    ...
    concat: {
      ...
      dist: {
        ...
        dest:
          '<%= config.distFolder %><%= config.appName %>-angular-libs.min.js',
      },
      dev: {
        src: [
    ...
    copy: {
      dist: {
        expand: true,
        src: [
          '<%= config.bowerFolder %>angular-sanitize/angular-sanitize.min.js.map',
  ...
  // grunt.registerTask('default', ['ngAnnotate:addRemoveLibs', 'ngAnnotate:addRemove', /*'jshint:uses_defaults',*/
  //     'concat:dev', 'sass:dev', 'clean:postclean', 'copy:tinymceskin', 'copy:html', 'copy:cssinjector', 'copy:stylesbower', 'copy:codemirror',
  //     'copy:i18n', 'copy:index']); //, 'sass:dev']);

  // grunt.registerTask('dist', ['clean:initclean', 'ngAnnotate:addRemoveLibs', 'ngAnnotate:addRemove', /*'jshint:uses_defaults',*/
  ...
  grunt.registerTask('default', [
    'clean:initclean',
    'ngAnnotate:addRemoveLibs',
    'ngAnnotate:addRemove' /*'jshint:uses_defaults',*/,
    'uglify:build',
    'uglify:lib_build',
    'concat:dist',
    'sass:dev',
